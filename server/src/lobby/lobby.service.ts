...
export class LobbyService {
  constructor(
    ...
    @InjectRepository(ft_user)
    private readonly UserRepository: Repository<ft_user>,
...
*/
  ...
  async getMyChatList(id: string): Promise<{ title: string; num: number }[]> {
    ...
    const isMyChat = (chatTitle) => {
      for (let i = 0; i < info.chat_room_list.length; i++) {
        if (info.chat_room_list[i] === chatTitle) return true;
      }
      return false;
    };
  ...
  async enterChatRoom(chatRoomID: string, id: string, password: string) {
    ...
    if (
      ...
      const found_title = user_info.chat_room_list.find(
